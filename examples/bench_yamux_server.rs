//! Benchmark server using `tokio-yamux` (reference implementation).
//!
...
async fn main() -> anyhow::Result<()> {
    ...
    let _reporter = bench_common::spawn_bandwidth_reporter("server/tokio-yamux", counters.clone());

    let listener = TcpListener::bind(args.bind).await?;
    log::info!("bench (tokio-yamux) server listening on {}", args.bind);
...
async fn serve_one(tcp: TcpStream, peer: SocketAddr, args: Args, counters: Arc<bench_common::Counters>) -> anyhow::Result<()> {
    ...
    let mut session = tokio_yamux::Session::new(tcp, tokio_yamux::Config::default(), tokio_yamux::session::SessionType::Server);
