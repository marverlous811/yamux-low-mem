...
async fn main() {
    let listener = tokio::net::TcpListener::bind("127.0.0.1:8080").await.expect("Failed to bind listener");
        ...
        tokio::spawn(async move {
            ...
            while let Some(mut stream) = session.next().await {
                ...
                tokio::spawn(async move {
                    ...
                    let len = stream.read(&mut buf).await.expect("Failed to read from stream");
                    println!("Read {} bytes from stream", len);

                    // Echo the data back
                    stream.write_all(&buf[..len]).await.expect("Failed to write to stream");
